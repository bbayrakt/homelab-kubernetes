...
- Runner tool-cache pins (the CLIs without setup actions that the `pr-preview` workflow runs on the self-hosted runner: vcluster, argocd-diff-preview, gh, kind) → regex custom managers matching the job-env `*_VERSION` values in `.github/workflows/warm-tool-cache.yaml` (github-releases datasource). The warming workflow installs them version-stamped and root-owned into `/opt/tool-cache/manual` (via sudo), exposed through `manual/bin` symlinks on the runner `PATH`, so a bump is a single env value and the next warming run (push-triggered) installs the new version. kubectl and helm are NOT pinned there; they come from the setup actions, which cache into the persisted `RUNNER_TOOL_CACHE` (`/opt/tool-cache/runner`, job-writable by design; see `README.md` for the accepted trade-off).

**PR preview** (`.github/workflows/pr-preview.yaml`): on PRs touching `platform/`/`apps/`, a self-hosted ARC runner (`argocd-diff-runner`) 1) renders the base vs. target diff through a dedicated Argo CD instance that runs **inside the vCluster** (namespace `argocd-preview` inside the vCluster, installed idempotently by the workflow via `helm upgrade --install`), 2) deploys changed, allowlisted apps as `preview-pr-<N>-<app>` Applications (targetRevision `refs/pull/<N>/merge`, automated sync, resources finalizer), and 3) posts one comment with `output/diff.md` plus the per-app health report (`output/preview-report.md`, generated by `.github/scripts/pr-preview.py`). The allowlist (what is testable in a vCluster: no CRDs, no host infra) lives in the script; everything else is skipped with a reason. On PR close a cleanup job deletes the `preview-pr-<N>-*` Applications (prune removes the workloads). The runner SA `arc-runner` reaches the vCluster via the host Roles in `platform/argocd-diff-runner/rbac.yaml` (`argocd-diff-preview` namespace access is retired with the host diff instance; `vcluster-connect` gives read access to the `vcluster` namespace, incl. the exported `vc-vcluster` kubeconfig Secret). The runner PAT lives in the `arc-runner-auth` Secret (namespace `arc-runners`), fed from the `github_runner_token` SOPS secret via the addons unit. `argocd-diff-preview` connects to the in-vCluster Argo CD through the vCluster kubeconfig (`vcluster connect --server https://vcluster.vcluster.svc:443 --insecure`), port-forwards to the server, and authenticates with the initial admin password it reads from the cluster.
