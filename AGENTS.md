...
- `.github/workflows/pr-preview.yaml` tool binary pins → regex custom managers (`dag-andersen/argocd-diff-preview`, `cli/cli` gh, `loft-sh/vcluster` CLI, github-releases datasource) plus the in-vCluster Argo CD chart version (helm datasource, `argoproj.github.io/argo-helm`). The `helm/helm` CLI pin custom manager covers both `pre-commit.yaml` and `pr-preview.yaml`.
