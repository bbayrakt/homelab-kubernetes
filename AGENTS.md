...
- local `argocd-apps-check` hook (`.github/scripts/check-argocd-apps.py`): for every `Application` manifest under `platform/`/`apps/`, pulls its helm/OCI chart at `targetRevision` (or shallow-clones the repo for git-sourced charts, e.g. the vpa chart app) and renders it with `helm template --include-crds` (ArgoCD's default; opt-out via `helm.skipCrds`) using the manifest's release name, namespace, and values (skips when helm/PyYAML are missing)
